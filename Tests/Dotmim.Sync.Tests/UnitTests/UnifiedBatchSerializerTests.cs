...
namespace Dotmim.Sync.Tests.UnitTests
{
    public class UnifiedBatchSerializerTests : IDisposable
    {
        ...
        public async Task UnifiedBatchSerializer_SingleTable_SingleRow_ShouldSerializeCorrectly()
        {
            ...
            await serializer.WriteRowAsync(row, table, BatchOperationType.Update);
            ...
            Assert.Equal(table.Columns.Count + 1, containerSet.Tables[0].Rows[0].Length); // columns + _rs
            Assert.Equal("u", containerSet.Tables[0].Rows[0][^1]); // Last item should be operation type
        }
        ...
        public async Task UnifiedBatchSerializer_MultipleTables_ShouldSerializeInCorrectOrder()
        {
            ...
            for (int i = 1; i <= 3; i++)
            {
                await serializer.WriteRowAsync(CreateTestRow(table1, i), table1, BatchOperationType.Update);
            }
            ...
            for (int i = 1; i <= 5; i++)
            {
                await serializer.WriteRowAsync(CreateTestRow(table2, i), table2, BatchOperationType.Update);
            }
            await serializer.CloseCurrentTableAsync();
            ...
            for (int i = 1; i <= 2; i++)
            {
                await serializer.WriteRowAsync(CreateTestRow(table3, i), table3, BatchOperationType.Update);
            }
        ...
        public async Task UnifiedBatchSerializer_MixedOperationTypes_ShouldSerializeCorrectOperationTypes()
        {
            ...
            await serializer.WriteRowAsync(CreateTestRow(table, 1), table, BatchOperationType.Insert);
            await serializer.WriteRowAsync(CreateTestRow(table, 2), table, BatchOperationType.Update);

            var deleteRow = CreateTestRow(table, 3);
            deleteRow.RowState = SyncRowState.Deleted;
            await serializer.WriteRowAsync(deleteRow, table, BatchOperationType.Delete);
            ...
            Assert.Equal("i", containerSet.Tables[0].Rows[0][^1]); // Insert
            Assert.Equal("u", containerSet.Tables[0].Rows[1][^1]); // Update
            Assert.Equal("d", containerSet.Tables[0].Rows[2][^1]); // Delete
        }
        ...
        public async Task UnifiedBatchSerializer_GetCurrentFileSizeKB_ShouldTrackBytesWritten()
        {
            ...
            for (int i = 1; i <= 100; i++)
            {
                await serializer.WriteRowAsync(CreateTestRow(table, i, "LongDataString"), table, BatchOperationType.Update);
            }
        ...
        [Fact]
        public async Task UnifiedBatchSerializer_ColumnsIncludeBatchingColumn_ShouldHaveRsColumn()
        {
            // Arrange
            var table = CreateTestTable("Product");
            var filePath = Path.Combine(this.tempDirectory, "test.json");
            ...
            await serializer.OpenFileAsync(filePath);
            await serializer.OpenTableAsync(table);
            await serializer.WriteRowAsync(CreateTestRow(table, 1), table, BatchOperationType.Update);
            await serializer.CloseFileAsync();
            ...
            var containerTable = containerSet.Tables[0];
            Assert.Equal(table.Columns.Count + 1, containerTable.Columns.Count); // Original columns + _rs
            Assert.Equal("_rs", containerTable.Columns[^1].ColumnName);
            Assert.Equal("String", containerTable.Columns[^1].TypeName);
        }
        ...
        public async Task UnifiedBatchSerializer_LargeTable_ExceedingSize_ShouldTrackCorrectly()
        {
            ...
            for (int i = 1; i <= 1000; i++)
            {
                ...
                var currentSize = await serializer.WriteRowAsync(row, table, BatchOperationType.Update);
