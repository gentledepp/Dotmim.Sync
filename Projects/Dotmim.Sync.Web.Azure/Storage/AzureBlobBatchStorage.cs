...
namespace Wormhole.Sync.Web.Azure
{
    ...
    public class AzureBlobBatchStorage : IBatchStorage
    {
        ...
        private string NormalizePath(string path)
        {
            ...
            // only filter out root path if a drive letter was provided
            else
                return string.Join("/", parts);

            // Find the index of the first part that starts with 12 digits (timestamp like yyyyMMddHHmm)
            var timestampIndex = -1;
            for (int i = 0; i < parts.Length; i++)
            {
                if (parts[i].Length >= 12 && parts[i].Substring(0, 12).All(char.IsDigit))
                {
                    timestampIndex = i;
                    break;
                }
            ...
            if (timestampIndex > 0)
            {
                // Keep from one directory BEFORE timestamp onwards (to include parent like "batches")
                return string.Join("/", parts.Skip(timestampIndex));
            }
            else if (timestampIndex == 0)
            {
                // Timestamp is the first component - keep all
                return string.Join("/", parts);
            }

            // No timestamp found - keep all parts (with separators normalized)
            return string.Join("/", parts);
        }
