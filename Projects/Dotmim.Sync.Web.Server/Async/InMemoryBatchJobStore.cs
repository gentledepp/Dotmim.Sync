...
namespace Wormhole.Sync.Web.Server.Async
{
    ...
    public class InMemoryBatchJobStore : IBatchJobStore
    {
        ...
        public Task<BatchCreationJobParameters> GetParametersAsync(string jobId, CancellationToken cancellationToken = default)
        {
            ...
            result?.ServerScopeInfo?.Schema?.EnsureSchema();
        ...
        /// <summary>
        /// Gets all job statuses in the store. Useful for testing and monitoring.
        /// </summary>
        /// <returns>Dictionary of job IDs to their statuses.</returns>
        public IDictionary<string, BatchCreationJobStatus> GetAllStatuses()
            => new Dictionary<string, BatchCreationJobStatus>(this.statuses);
    }
}
