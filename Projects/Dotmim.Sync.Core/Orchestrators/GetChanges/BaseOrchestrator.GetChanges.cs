   ...
   public abstract partial class BaseOrchestrator
   {
                           ...
                           HashSet<string> reinitTables = null)
      {
         try
         {
            ...
            if (!string.IsNullOrEmpty(batchInfo.DirectoryRoot))
               await this.BatchStorage.EnsureDirectoryExistsAsync(batchInfo.DirectoryRoot, cancellationToken).ConfigureAwait(false);
            ...
            var unifiedSerializer = new Serialization.UnifiedBatchSerializer(this.BatchStorage);
            ...
            if (unifiedSerializer.IsOpen)
            {
               ...
               if (batchIndex >= 0)
               {
                  ...
                  await this.InterceptAsync(new BatchChangesCreatedArgs(context, batchInfo, lastBatchInfo, null, null, SyncRowState.None, connection, transaction), progress, cancellationToken).ConfigureAwait(false);
               }
            }
                           ...
                           HashSet<string> reinitTables = null)
      {
         ...
         try
         {
            ...
            await this.EnsureLastBatchInfoAsync(scopeInfo, context, batchInfo, lstAllBatchPartInfos, schemaTables, cancellationToken).ConfigureAwait(false);
               ...
               try
               {
                  ...
                  while (await dataReader.ReadAsync(cancellationToken).ConfigureAwait(false))
                  {
                     ...
                     if (!unifiedSerializer.IsOpen)
                     {
                        ...
                        await unifiedSerializer.OpenFileAsync(directoryPath, batchPartFileName, cancellationToken).ConfigureAwait(false);
                     ...
                     if (newSizeKB > this.Options.BatchSize)
                     {
                        ...
                        if (batchPartInfos.Count > 0)
                        {
                           ...
                           await this.InterceptAsync(new BatchChangesCreatedArgs(context, batchInfo, completedBatchPartInfo, null, null, SyncRowState.None, connection, transaction), progress, cancellationToken).ConfigureAwait(false);
                        }
                     }
                  }
               }
         ...
         IProgress<ProgressArgs> progress, CancellationToken cancellationToken)
      {
         try
         {
            ...
            {
               await this.BatchStorage.WriteBatchPartAsync(directoryPath, batchPartFileName, dataStream, cancellationToken).ConfigureAwait(false);
            }
            ...
            await this.InterceptAsync(new BatchChangesCreatedArgs(context, batchInfo, batchPartInfo, null, null, SyncRowState.None, connection, transaction), progress, cancellationToken).ConfigureAwait(false);
         ...
         IProgress<ProgressArgs> progress, CancellationToken cancellationToken)
      {
         ...
         var localSerializerModified = new LocalJsonSerializer(this.BatchStorage, this, context);
         var localSerializerDeleted = new LocalJsonSerializer(this.BatchStorage, this, context);
         try
         {
            var setupTable = scopeInfo.Setup.Tables[syncTable.TableName, syncTable.SchemaName];
            ...
            if (!args.Cancel && args.Command != null)
            {
               ...
               if (batchPartInfoUpserts != null || batchPartInfoDeleted != null)
               {
                  ...
                  {
                     await closeSerializer(localSerializerDeleted, new BatchChangesCreatedArgs(context, batchInfo, batchPartInfoDeleted, schemaChangesTable, tableChangesSelected, SyncRowState.Deleted, connection, transaction)).ConfigureAwait(false);
                     await closeSerializer(localSerializerModified, new BatchChangesCreatedArgs(context, batchInfo, batchPartInfoUpserts, schemaChangesTable, tableChangesSelected, SyncRowState.Modified, connection, transaction)).ConfigureAwait(false);
                  }
                  else
                  {
                     await closeSerializer(localSerializerModified, new BatchChangesCreatedArgs(context, batchInfo, batchPartInfoUpserts, schemaChangesTable, tableChangesSelected, SyncRowState.Modified, connection, transaction)).ConfigureAwait(false);
               ...
               if (localSerializerModified.IsOpen)
               {
                  ...
                  await this.InterceptAsync(new BatchChangesCreatedArgs(context, batchInfo, batchPartInfoUpserts, schemaChangesTable, tableChangesSelected, SyncRowState.Modified, connection, transaction), progress, cancellationToken).ConfigureAwait(false);
               }

               if (localSerializerDeleted.IsOpen)
               {
                  await localSerializerDeleted.CloseFileAsync().ConfigureAwait(false);
                  await this.InterceptAsync(new BatchChangesCreatedArgs(context, batchInfo, batchPartInfoDeleted, schemaChangesTable, tableChangesSelected, SyncRowState.Deleted, connection, transaction), progress, cancellationToken).ConfigureAwait(false);
               }
            }
            ...
            foreach (var bpi in batchPartInfos.ToArray())
            {
               ...
               if (fullPath != null && bpi.RowsCount == 0)
               {
         ...
         IProgress<ProgressArgs> progress, CancellationToken cancellationToken)
      {
         ...
         {
            var (batchPartInfoFullPath, batchPartFileName) = batchInfo.GetNewBatchPartInfoPath(schemaChangesTable, index, localJsonSerializer.FileExtension, ext);
            var directoryPath = Path.GetDirectoryName(batchPartInfoFullPath);
            var fileName = Path.GetFileName(batchPartInfoFullPath);
            await localJsonSerializer.OpenFileAsync(directoryPath, fileName, schemaChangesTable, syncRow.RowState).ConfigureAwait(false);
         ...
         {
            await localJsonSerializer.CloseFileAsync().ConfigureAwait(false);
            await this.InterceptAsync(new BatchChangesCreatedArgs(context, batchInfo, batchPartInfo, schemaChangesTable, tableChangesSelected, syncRow.RowState, connection, transaction), progress, cancellationToken).ConfigureAwait(false);
         }
      ...
      /// </summary>
      internal async Task EnsureLastBatchInfoAsync(ScopeInfo scopeInfo, SyncContext context, BatchInfo batchInfo, IEnumerable<BatchPartInfo> lstAllBatchPartInfos, IEnumerable<SyncTable> schemaTables, CancellationToken cancellationToken = default)
      {
         try
         {
            ...
            {
               var directoryPath = batchInfo.GetDirectoryFullPath();
               await this.BatchStorage.DeleteBatchPartAsync(directoryPath, bpi.FileName, cancellationToken).ConfigureAwait(false);
            }
