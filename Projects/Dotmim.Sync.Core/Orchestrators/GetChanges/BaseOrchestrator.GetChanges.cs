...
namespace Dotmim.Sync
{
    ...
    public abstract partial class BaseOrchestrator
    {
                ...
                if (!args.Cancel && args.Command != null)
                {
                    ...
                    try
                    {
                        ...
                        while (await dataReader.ReadAsync(cancellationToken).ConfigureAwait(false))
                        {
                            ...
                            // Determine operation type from SyncRowState
                            var operationType = syncRow.RowState switch
                            {
                                SyncRowState.Deleted or SyncRowState.RetryDeletedOnNextSync or SyncRowState.ApplyDeletedFailed => BatchOperationType.Delete,
                                SyncRowState.Modified or SyncRowState.RetryModifiedOnNextSync or SyncRowState.ApplyModifiedFailed => BatchOperationType.Update,
                                _ => BatchOperationType.Update
                            };
                            ...
                            // Write row and get new file size
                            var batchSizeInBytes = await unifiedSerializer.WriteRowAsync(syncRow, schemaChangesTable, operationType).ConfigureAwait(false);
        ...
        {
            // Convert SyncRow to object array using the correct length (number of columns, not buffer size)
            var rowData = new object[syncRow.Length];
            for (int i = 0; i < syncRow.Length; i++)
            {
                rowData[i] = syncRow[i];
            }
            ...
            var operationType = syncRow.RowState switch
            {
                SyncRowState.Deleted or SyncRowState.RetryDeletedOnNextSync or SyncRowState.ApplyDeletedFailed => BatchOperationType.Delete,
                SyncRowState.Modified or SyncRowState.RetryModifiedOnNextSync or SyncRowState.ApplyModifiedFailed => BatchOperationType.Update,
                _ => BatchOperationType.Update // Default to update for any other states
            };
            containerTable.AddRowWithOperationType(rowData, operationType);
